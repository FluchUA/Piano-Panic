...
// 1. Выводим текст-инструкцию на экран
        this.add.text(50, 50, 'Piano Test App', { fontSize: '24px', color: '#fff' });

        // 2. Создаем текстовую «кнопку»
        const testButton = this.add.text(100, 200, '[ НАЖМИ МЕНЯ ]', { 
            fontSize: '32px', 
            color: '#ff0000',
            backgroundColor: '#222',
            padding: { x: 20, y: 10 }
        ...
        // 3. Делаем текст интерактивным
        testButton.setInteractive({ useHandCursor: true });

        // Создаем самый простой синтезатор Tone.js
        const synth = new Tone.Synth().toDestination();

        // 4. Вешаем событие клика/тапа
        testButton.on('pointerdown', async () => {
            // КРИТИЧЕСКИ ВАЖНО: Разблокируем аудио-контекст браузера при первом клике
            if (Tone.context.state !== 'running') {
                await Tone.start();
                console.log('Аудио-контекст запущен!');
            }

            // Играем ноту До (C4) длительностью в 1/4 такта
            synth.triggerAttackRelease('C4', '8n');

            // Небольшой визуальный эффект нажатия кнопки
            testButton.setStyle({ color: '#00ff00' });
            this.time.delayedCall(100, () => testButton.setStyle({ color: '#ff0000' }));
        });
